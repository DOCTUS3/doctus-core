...
private[util] object DoctusVectorImpl {
  ...
  trait AbstractDoctusVector extends DoctusVector {
    def +(v: DoctusVector): DoctusVector = add(this, v)
    def -(v: DoctusVector): DoctusVector = sub(this, v)
    def *(s: Double): DoctusVector = mulScalar(this, s)
    def norm: Double = norm1(this)
    def toUnit: DoctusVector = toUnit1(this)
    def rot(rad: Double): DoctusVector = rot1(this, rad)
  }
